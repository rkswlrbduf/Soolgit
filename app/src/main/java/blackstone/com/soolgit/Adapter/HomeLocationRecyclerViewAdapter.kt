...
class HomeLocationRecyclerViewAdapter(val context : Context, val list: ArrayList<HotPlaceStoreData>?) : BaseQuickAdapter<HotPlaceStoreData, BaseViewHolder>(R.layout.main_home_content_location_recyclerview_row, list) {

    override fun convert(helper: BaseViewHolder, item: HotPlaceStoreData) {
        helper.setText(R.id.main_home_content_location_row_title_textview, item.StoreName)
        Glide.with(context)
                .load(item.StoreImg)
